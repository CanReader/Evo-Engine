...
pub fn sbx_crossover(
    ...
    for i in 0..n {
        ...
        // Calculate beta_q for each child
...
pub fn arithmetic_crossover(
    ...
    let c1 = p1.iter().zip(p2).map(|(a, b)| alpha * a + (1.0 - alpha) * b).collect();
    let c2 = p1.iter().zip(p2).map(|(a, b)| (1.0 - alpha) * a + alpha * b).collect();
    (c1, c2)
}
...
pub fn undx_crossover(
    ...
    // Displacement of p3 from midpoint, projected onto orthogonal complement
    let diff: Vec<f64> = p3.iter().zip(&midpoint).map(|(a, b)| a - b).collect();
    let proj_len: f64 = diff.iter().zip(&d_unit).map(|(a, b)| a * b).sum();
    let orthogonal: Vec<f64> = diff.iter().zip(&d_unit).map(|(a, b)| a - proj_len * b).collect();
    let orth_len = orthogonal.iter().map(|x| x * x).sum::<f64>().sqrt();

    // Child = midpoint + sigma_xi * d_len * N(0,1) * d_unit + sigma_eta * orth_len * N(0,I)_orth
